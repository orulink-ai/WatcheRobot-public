...
Serial auto-pairing in `tools/hardware_smoke.py` is only for maintainer bench firmware with Debug CLI enabled. Production
users should pair through the code displayed by SDK Control App / 串口自动配对仅用于维护者台架固件，
