...
UDP Discovery 始终监听 `0.0.0.0:37021`，因此 ESP32 的受限广播可以到达全部 IPv4 接口。`host` 参数只控制
WebSocket 的监听地址；仅在需要限制该监听地址时传入具体 IP。
...
| 固件 | `V3.1` 或 Release 说明中标注支持 SDK 的版本，且 Launcher 中存在 **SDK Control App** |
| 网络 | 电脑与机器人位于同一局域网；本机防火墙允许 UDP `37021` 和 TCP `8766` |
| Python | CPython 3.10、3.11 或 3.12 |

请使用 WatcheRobot 主仓 GitHub Releases 中标注支持 SDK Control App 的 ESP32-S3 固件包。公开仓不提供 ESP32-S3 固件源码构建入口；如果当前 Release 尚未包含 SDK Control App，请等待匹配版本或联系维护者确认固件包。
