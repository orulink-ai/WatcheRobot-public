...
| Firmware | `V3.1` or a Release version marked as SDK-compatible, with **SDK Control App** in the Launcher |
...
Use the ESP32-S3 firmware package from the WatcheRobot main repository Releases when it is marked as SDK Control App compatible. This public repository does not provide an ESP32-S3 firmware source build entrypoint; if the current Release does not include SDK Control App, wait for a matching bundle or confirm the firmware package with the maintainers.
